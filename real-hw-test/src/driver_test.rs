...
//!
//! Every discovered candidate is exercised, starting with `init()`, whose
//! built-in presence check separates an absent UART from a driver failure.
...
impl Result {
    /// Summarizes one UART's outcome for the final on-screen list.
    pub fn status(&self) -> &'static str {
        if !self.passed {
            "FAIL"
        } else if self.connection_warning || self.interactive_skipped {
            "WARN"
        } else {
            "PASS"
        }
...
/// Exercises every candidate through the public driver API.
